...
type RequirementEvidence = { requirement: string; evidence: string };
type NegotiationStatus = "Not discussed" | "In discussion" | "Agreed" | "Declined";
type OfferDecisionOutcome = "Undecided" | "Accepted" | "Declined" | "More time requested";
type NegotiationTopic = {
  id: string;
  topic: "Base salary" | "Bonus" | "Equity" | "Title / level" | "Work model" | "Start date";
...
type OfferDetails = {
  ...
  verifiedAt: string;
  negotiationTopics: NegotiationTopic[];
  decisionOutcome: OfferDecisionOutcome;
...
const emptyApplication: ApplicationDraft = {
  ...
  offerDetails: { baseSalary: "", currency: "ILS", bonus: "", equity: "", benefits: "", titleLevel: "", startDate: "", responseDeadline: "", workModel: "", additionalTerms: "", verifiedAt: "", negotiationTopics: [
    ...
    { id: "equity", topic: "Equity", status: "Not discussed", original: "", target: "", outcome: "" },
    { id: "title-level", topic: "Title / level", status: "Not discussed", original: "", target: "", outcome: "" },
    { id: "work-model", topic: "Work model", status: "Not discussed", original: "", target: "", outcome: "" },
    { id: "start-date", topic: "Start date", status: "Not discussed", original: "", target: "", outcome: "" },
  ], decisionOutcome: "Undecided", decisionReason: "", decisionDate: "" },
  capturedAt: "",
  salary: "",
  budgetRange: "",
  salaryCurrency: "ILS",
...
function normalizeApplication(value: Partial<Application>): Application {
  const storedOffer = value.offerDetails;
  const negotiationStatuses: NegotiationStatus[] = ["Not discussed", "In discussion", "Agreed", "Declined"];
  const decisionOutcomes: OfferDecisionOutcome[] = ["Undecided", "Accepted", "Declined", "More time requested"];
  const negotiationTopics = emptyApplication.offerDetails.negotiationTopics.map((fallbackTopic) => {
    const storedTopic = Array.isArray(storedOffer?.negotiationTopics)
      ? storedOffer.negotiationTopics.find((item) => item?.topic === fallbackTopic.topic)
      : null;
    return {
      ...fallbackTopic,
      ...(storedTopic || {}),
      id: storedTopic?.id || fallbackTopic.id,
      status: negotiationStatuses.includes(storedTopic?.status as NegotiationStatus) ? storedTopic!.status : "Not discussed",
    };
  });
  return {
    ...emptyApplication,
    ...
    requirementEvidence: syncRequirementEvidence(
      value.requirements || "",
      Array.isArray(value.requirementEvidence) ? value.requirementEvidence
        .filter((item): item is RequirementEvidence => Boolean(item && typeof item.requirement === "string" && typeof item.evidence === "string"))
        .map((item) => ({ requirement: item.requirement, evidence: item.evidence })) : [],
    ),
    offerDetails: {
      ...emptyApplication.offerDetails,
      ...(storedOffer || {}),
      currency: ["ILS", "USD", "EUR", "GBP", "Other"].includes(storedOffer?.currency || "") ? storedOffer!.currency : "ILS",
      negotiationTopics,
      decisionOutcome: decisionOutcomes.includes(storedOffer?.decisionOutcome as OfferDecisionOutcome) ? storedOffer!.decisionOutcome : "Undecided",
    },
...
function syncRequirementEvidence(requirements: string, current: RequirementEvidence[]) {
  const existing = new Map(current.map((item) => [item.requirement.trim().toLowerCase(), item.evidence]));
  return requirements
    .split(/\n|•/)
    .map((item) => item.trim())
    .filter(Boolean)
    .slice(0, 8)
    .map((requirement) => ({ requirement, evidence: existing.get(requirement.toLowerCase()) || "" }));
}
...
function offerReadiness(application: Application, language: "en" | "he") {
  const offer = application.offerDetails;
  const fields = [offer.baseSalary, offer.benefits, offer.titleLevel, offer.startDate, offer.responseDeadline, offer.workModel];
  const missingLabels = [
    language === "he" ? "שכר מוצע" : "offered salary",
    language === "he" ? "הטבות" : "benefits",
    language === "he" ? "תפקיד או דרגה" : "title or level",
    language === "he" ? "תאריך התחלה" : "start date",
    language === "he" ? "מועד החלטה" : "decision deadline",
    language === "he" ? "מודל עבודה" : "work model",
  ].filter((_, index) => !fields[index]);
  const score = Math.round(((fields.length - missingLabels.length) / fields.length) * 100);
  const deadline = offer.responseDeadline ? new Date(`${offer.responseDeadline}T23:59:59`) : null;
  const daysRemaining = deadline ? Math.ceil((deadline.getTime() - Date.now()) / 86400000) : null;
  const state = daysRemaining !== null && daysRemaining < 0
    ? (language === "he" ? "ההחלטה באיחור" : "Decision overdue")
    : daysRemaining !== null && daysRemaining <= 3
      ? (language === "he" ? "מועד ההחלטה מתקרב" : "Deadline approaching")
      : missingLabels.length
        ? (language === "he" ? "התנאים אינם מלאים" : "Terms incomplete")
        : (language === "he" ? "מוכן לבדיקה" : "Ready to review");
  return { score, missingLabels, daysRemaining, state };
}

function negotiationTopicLabel(topic: NegotiationTopic["topic"], language: "en" | "he") {
  if (language === "en") return topic;
  return ({
    "Base salary": "שכר בסיס",
    Bonus: "בונוס",
    Equity: "הון / מניות",
    "Title / level": "תפקיד / דרגה",
    "Work model": "מודל עבודה",
    "Start date": "תאריך התחלה",
  } as Record<NegotiationTopic["topic"], string>)[topic];
}

function negotiationStatusLabel(status: NegotiationStatus, language: "en" | "he") {
  if (language === "en") return status;
  return ({
    "Not discussed": "טרם נדון",
    "In discussion": "בדיון",
    Agreed: "אושר",
    Declined: "נדחה",
  } as Record<NegotiationStatus, string>)[status];
}

function offerDecisionLabel(outcome: OfferDecisionOutcome, language: "en" | "he") {
  if (language === "en") return outcome;
  return ({
    Undecided: "טרם התקבלה החלטה",
    Accepted: "ההצעה התקבלה",
    Declined: "ההצעה נדחתה",
    "More time requested": "התבקשה הארכה",
  } as Record<OfferDecisionOutcome, string>)[outcome];
}

function applicationEvidenceContext(application: Application, language: "en" | "he") {
  const mapped = application.requirementEvidence.length
    ? application.requirementEvidence
    : syncRequirementEvidence(application.requirements, []);
  if (!mapped.length) return language === "he" ? "טרם נשמרו דרישות למשרה." : "No role requirements have been saved yet.";
  return mapped.map((item, index) => {
    const evidence = item.evidence.trim() || (language === "he" ? "פער: נדרשת דוגמה או דרך להסביר את הפער." : "Gap: add a proof point or prepare how to explain the gap.");
    return `${index + 1}. ${item.requirement}\n   ${language === "he" ? "ראיה" : "Evidence"}: ${evidence}`;
  }).join("\n");
}
...
export default function Home() {
  ...
  const [offerDecisionDraft, setOfferDecisionDraft] = useState("");
  ...
  const [expandedTools, setExpandedTools] = useState({ studio: false, social: false, cv: false, interview: false, offer: false, search: true, analytics: true });
  ...
  const [showTodayOverflow, setShowTodayOverflow] = useState(false);
  const [actionEvents, setActionEvents] = useState<ActionEvent[]>([]);
  const messageDraftRef = useRef<HTMLTextAreaElement | null>(null);
  const todayOverflowRef = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    if (!showTodayOverflow) return;
    const closeTodayOverflow = (event: MouseEvent | KeyboardEvent) => {
      if (event instanceof KeyboardEvent && event.key === "Escape") {
        setShowTodayOverflow(false);
        return;
      }
      if (event instanceof MouseEvent && todayOverflowRef.current && !todayOverflowRef.current.contains(event.target as Node)) setShowTodayOverflow(false);
    };
    document.addEventListener("mousedown", closeTodayOverflow);
    document.addEventListener("keydown", closeTodayOverflow);
    return () => {
      document.removeEventListener("mousedown", closeTodayOverflow);
      document.removeEventListener("keydown", closeTodayOverflow);
    };
  }, [showTodayOverflow]);
  ...
  const workspaceOfferReadiness = workspaceApplication?.status === "Offer" ? offerReadiness(workspaceApplication, language) : null;
  const workspaceNegotiation = workspaceApplication?.status === "Offer" ? {
    active: workspaceApplication.offerDetails.negotiationTopics.filter((item) => item.status === "In discussion").length,
    resolved: workspaceApplication.offerDetails.negotiationTopics.filter((item) => item.status === "Agreed" || item.status === "Declined").length,
    tracked: workspaceApplication.offerDetails.negotiationTopics.filter((item) => item.status !== "Not discussed" || item.target.trim() || item.outcome.trim()).length,
  } : null;
  ...
  const searchProgress = useMemo(() => {
    const activeOpportunities = applications.filter((item) => !["Rejected", "Withdrawn"].includes(item.status)).length;
    const conversations = contacts.filter((item) => item.lastContactDate || item.eventDateTime).length;
    const interviews = applications.filter((item) => item.status === "Interview" || item.status === "Offer" || item.processStages.some((stage) => /interview|ראיון|screen|portfolio|panel|שיחת סינון/i.test(stage.name))).length;
    const weekAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
    const meaningfulEntities = new Set<string>();
    applications.forEach((item) => {
      if (item.appliedDate && !Number.isNaN(new Date(item.appliedDate).getTime()) && new Date(item.appliedDate).getTime() >= weekAgo) meaningfulEntities.add(`application-${item.id}`);
    });
    contacts.forEach((item) => {
      if (item.lastContactDate && !Number.isNaN(new Date(item.lastContactDate).getTime()) && new Date(item.lastContactDate).getTime() >= weekAgo) meaningfulEntities.add(`contact-${item.id}`);
    });
    actionEvents.forEach((event) => {
      if (Number.isNaN(new Date(event.occurredAt).getTime()) || new Date(event.occurredAt).getTime() < weekAgo) return;
      if (["application_created", "application_action_completed"].includes(event.name)) meaningfulEntities.add(`application-${event.targetId}`);
      if (event.name === "contact_created") meaningfulEntities.add(`contact-${event.targetId}`);
    });
    const meaningfulMoves = meaningfulEntities.size;
    const isEmpty = applications.length === 0 && contacts.length === 0;
    const statement = isEmpty
      ? (language === "he" ? "ההתקדמות תופיע כאן לאחר הוספת ההזדמנות או השיחה הראשונה." : "Progress will appear here after you add your first opportunity or conversation.")
      : meaningfulMoves > 0
        ? (language === "he" ? `${meaningfulMoves} ${meaningfulMoves === 1 ? "התקדמות משמעותית תועדה" : "התקדמויות משמעותיות תועדו"} השבוע` : `${meaningfulMoves} meaningful ${meaningfulMoves === 1 ? "move" : "moves"} recorded this week`)
        : interviews > 0
          ? (language === "he" ? `${interviews} ${interviews === 1 ? "תהליך נמצא" : "תהליכים נמצאים"} בשלב ראיון` : `${interviews} ${interviews === 1 ? "opportunity is" : "opportunities are"} currently at interview stage`)
          : activeOpportunities > 0
            ? (language === "he" ? `${activeOpportunities} ${activeOpportunities === 1 ? "הזדמנות פעילה מוכנה" : "הזדמנויות פעילות מוכנות"} לצעד הבא` : `${activeOpportunities} active ${activeOpportunities === 1 ? "opportunity is" : "opportunities are"} ready for a next move`)
            : (language === "he" ? "השיחות שתועדו ממשיכות לבנות את החיפוש." : "Your recorded conversations are continuing to build the search.");
    return { activeOpportunities, conversations, interviews, meaningfulMoves, isEmpty, statement };
  }, [actionEvents, applications, contacts, language]);
  ...
  function navigateToSection(target: string) {
    const tool = target === "message-studio" ? "studio" : target === "social-studio" ? "social" : target === "cv-lab" ? "cv" : target === "interview-prep" ? "interview" : target === "offer-decision" ? "offer" : target === "job-search" ? "search" : target === "analytics" ? "analytics" : null;
  ...
  const nextBestActions = useMemo(() => {
    ...
    return actions.sort((a, b) => b.score - a.score).filter((action) => {
      const entityKey = action.applicationId ? `application-${action.applicationId}` : action.contactId ? `contact-${action.contactId}` : action.id;
  ...
  const primaryActionApplication = nextBestActions[0]?.applicationId
    ...
    : null;
  const primaryActionContact = nextBestActions[0]?.contactId
    ? contacts.find((item) => item.id === nextBestActions[0].contactId) || null
    : null;

  const todayActionPresentation = useMemo(() => {
    const action = nextBestActions[0];
    const joinContext = (...parts: string[]) => parts.filter((part) => part.trim()).join(language === "he" ? " ב־" : " at ");
    if (!action || (!applications.length && !contacts.length)) return {
      headline: language === "he" ? "הוספת ההזדמנות הראשונה" : "Add your first opportunity",
      reason: language === "he" ? "הוסיפו הזדמנות כדי ש־CARVIO יוכל להמליץ על הצעד הבא." : "Add an opportunity so CARVIO can recommend a useful next move.",
      cta: language === "he" ? "הוספת הזדמנות" : "Add opportunity",
      canComplete: false,
    };
    if (primaryActionApplication) {
      const application = primaryActionApplication;
      const roleAtCompany = joinContext(application.role, application.company) || (language === "he" ? "המועמדות" : "your application");
      if (action.id.startsWith("app-event-")) return {
        headline: language === "he" ? `הכנה ל${application.eventType || "ראיון"} עבור ${roleAtCompany}` : `Prepare for your ${application.eventType?.toLowerCase() || "interview"} for ${roleAtCompany}`,
        reason: application.eventDateTime ? (language === "he" ? `המועד נקבע ל־${formatDate(application.eventDateTime, true)}. כדאי להשלים את ההכנה לפניו.` : `Scheduled for ${formatDate(application.eventDateTime, true)} · complete your preparation beforehand.`) : action.detail,
        cta: language === "he" ? "פתיחת ההכנה" : "Open preparation",
        canComplete: false,
      };
      if (action.id.startsWith("app-due-") || action.id.startsWith("app-wait-")) return {
        headline: language === "he" ? `ביצוע פעולת המשך עבור ${roleAtCompany}` : `Follow up on your ${roleAtCompany} application`,
        reason: application.nextStepDue ? (language === "he" ? `פעולת ההמשך הייתה מיועדת ל־${formatDate(application.nextStepDue)}.` : `The follow-up was due ${formatDate(application.nextStepDue)}.`) : action.detail,
        cta: language === "he" ? "כתיבת הודעת המשך" : "Send follow-up",
        canComplete: true,
      };
      if (action.id.startsWith("app-contact-")) return {
        headline: language === "he" ? `הוספת קשר רלוונטי עבור ${roleAtCompany}` : `Add a relevant contact for ${roleAtCompany}`,
        reason: language === "he" ? "להזדמנות בעדיפות גבוהה עדיין אין איש קשר מקושר." : "This high-priority opportunity does not have a linked contact yet.",
        cta: language === "he" ? "הוספת קשר" : "Add contact",
        canComplete: false,
      };
      if (action.id.startsWith("app-plan-")) return {
        headline: language === "he" ? `הגדרת הצעד הבא עבור ${roleAtCompany}` : `Define the next step for ${roleAtCompany}`,
        reason: language === "he" ? "למועמדות הפעילה עדיין אין פעולה ברורה." : "This active application does not have a clear next step yet.",
        cta: language === "he" ? "הגדרת צעד הבא" : "Set next step",
        canComplete: false,
      };
      if (action.intent === "recover") return {
        headline: language === "he" ? `איפוס עדין לאחר התהליך עבור ${roleAtCompany}` : `Take a gentle reset after ${roleAtCompany}`,
        reason: action.detail,
        cta: language === "he" ? "פתיחת האיפוס" : "Open reset",
        canComplete: false,
      };
      return { headline: homepageAction.title, reason: action.detail || homepageAction.support, cta: language === "he" ? "בדיקת המועמדות" : "Review application", canComplete: true };
    }
    if (primaryActionContact) {
      const contactContext = [primaryActionContact.name, primaryActionContact.company].filter(Boolean).join(language === "he" ? " · " : " at ");
      return {
        headline: language === "he" ? `חזרה אל ${contactContext}` : `Follow up with ${contactContext}`,
        reason: primaryActionContact.nextActionDue ? (language === "he" ? `פעולת הנטוורקינג הייתה מיועדת ל־${formatDate(primaryActionContact.nextActionDue)}.` : `The networking follow-up was due ${formatDate(primaryActionContact.nextActionDue)}.`) : action.detail,
        cta: language === "he" ? "פתיחת השיחה" : "Open conversation",
        canComplete: false,
      };
    }
    if (action.intent === "recover") return { headline: action.label, reason: action.detail, cta: language === "he" ? "פתיחת האיפוס" : "Open reset", canComplete: false };
    return { headline: action.label, reason: action.detail, cta: language === "he" ? "לבדיקת המועמדויות" : "Review applications", canComplete: false };
  }, [applications.length, contacts.length, homepageAction.support, homepageAction.title, language, nextBestActions, primaryActionApplication, primaryActionContact]);

  const attentionPresentations = useMemo(() => nextBestActions.slice(1).map((action) => {
    const application = action.applicationId ? applications.find((item) => item.id === action.applicationId) || null : null;
    const contact = action.contactId ? contacts.find((item) => item.id === action.contactId) || null : null;
    const validDate = (value: string) => value && !Number.isNaN(new Date(/^\d{4}-\d{2}-\d{2}$/.test(value) ? `${value}T12:00:00` : value).getTime());
    const applicationContext = application
      ? [application.role.trim(), application.company.trim()].filter(Boolean).join(language === "he" ? " ב־" : " at ") || (language === "he" ? "המועמדות" : "the application")
      : "";
    if (application && action.id.startsWith("app-event-")) return {
      action,
      headline: language === "he" ? `הכנה ל${application.eventType || "ראיון"} עבור ${applicationContext}` : `Prepare for the ${application.eventType?.toLowerCase() || "interview"} for ${applicationContext}`,
      reason: language === "he" ? "כדאי להשלים את ההכנה לפני המפגש." : "Complete the preparation before the meeting.",
      urgency: validDate(application.eventDateTime) ? formatDate(application.eventDateTime, true) : "",
      tone: "soon",
      cta: language === "he" ? "פתיחת ההכנה" : "Prepare",
      icon: "meeting",
    };
    if (application && action.id.startsWith("app-due-")) return {
      action,
      headline: language === "he" ? `ביצוע פעולת המשך עבור ${applicationContext}` : `Follow up on ${applicationContext}`,
      reason: validDate(application.nextStepDue) ? (language === "he" ? `פעולת ההמשך הייתה מיועדת ל־${formatDate(application.nextStepDue)}.` : `The follow-up was due ${formatDate(application.nextStepDue)}.`) : (language === "he" ? "פעולת ההמשך עדיין ממתינה לטיפול." : "The follow-up is still waiting for action."),
      urgency: language === "he" ? "באיחור" : "Overdue",
      tone: "overdue",
      cta: language === "he" ? "כתיבת הודעה" : "Follow up",
      icon: "overdue",
    };
    if (application && action.id.startsWith("app-wait-")) {
      const activityDate = application.lastActivityAt || application.appliedDate;
      const staleDays = validDate(activityDate) ? Math.max(0, Math.floor((Date.now() - new Date(activityDate).getTime()) / 86400000)) : 0;
      return {
        action,
        headline: language === "he" ? `בדיקת התהליך עבור ${applicationContext}` : `Review ${applicationContext}`,
        reason: staleDays ? (language === "he" ? `לא תועדה פעילות כבר ${staleDays} ימים.` : `No activity has been recorded for ${staleDays} days.`) : (language === "he" ? "התהליך ממתין ללא תנועה מתועדת." : "The process is waiting without recorded activity."),
        urgency: staleDays ? (language === "he" ? `${staleDays} ימים` : `${staleDays} days`) : "",
        tone: "neutral",
        cta: language === "he" ? "בדיקה" : "Review",
        icon: "review",
      };
    }
    if (application && action.id.startsWith("app-contact-")) return {
      action,
      headline: language === "he" ? `הוספת קשר רלוונטי עבור ${applicationContext}` : `Add a relevant contact for ${applicationContext}`,
      reason: language === "he" ? "להזדמנות בעדיפות גבוהה עדיין אין איש קשר מקושר." : "This high-priority opportunity does not have a linked contact.",
      urgency: language === "he" ? "עדיפות גבוהה" : "High priority",
      tone: "neutral",
      cta: language === "he" ? "הוספת קשר" : "Add contact",
      icon: "contact",
    };
    if (application && action.id.startsWith("app-plan-")) return {
      action,
      headline: language === "he" ? `הגדרת הצעד הבא עבור ${applicationContext}` : `Define the next step for ${applicationContext}`,
      reason: language === "he" ? "למועמדות הפעילה עדיין אין פעולה ברורה." : "This active application does not have a clear next step.",
      urgency: language === "he" ? "נדרש צעד הבא" : "Needs a next step",
      tone: "neutral",
      cta: language === "he" ? "הגדרת צעד" : "Set next step",
      icon: "plan",
    };
    if (contact) {
      const contactContext = [contact.name.trim(), contact.company.trim()].filter(Boolean).join(language === "he" ? " · " : " at ") || (language === "he" ? "איש הקשר" : "this contact");
      return {
        action,
        headline: language === "he" ? `חזרה אל ${contactContext}` : `Follow up with ${contactContext}`,
        reason: validDate(contact.nextActionDue) ? (language === "he" ? `פעולת הנטוורקינג הייתה מיועדת ל־${formatDate(contact.nextActionDue)}.` : `The networking follow-up was due ${formatDate(contact.nextActionDue)}.`) : (language === "he" ? "פעולת הנטוורקינג עדיין ממתינה." : "The networking follow-up is still waiting."),
        urgency: language === "he" ? "באיחור" : "Overdue",
        tone: "overdue",
        cta: language === "he" ? "פתיחת השיחה" : "Open conversation",
        icon: "contact",
    ...
    if (application && action.intent === "recover") return {
      action,
      headline: language === "he" ? `איפוס עדין לאחר התהליך עבור ${applicationContext}` : `Take a gentle reset after ${applicationContext}`,
      reason: action.detail,
      urgency: "",
      tone: "neutral",
      cta: language === "he" ? "פתיחת האיפוס" : "Open reset",
    ...
    return {
      ...
      urgency: "",
      tone: "neutral",
      cta: language === "he" ? "פתיחה" : "Open",
      icon: "review",
    };
  }), [applications, contacts, language, nextBestActions]);
  ...
  function smartCaptureApplication() {
    ...
    const detectedRequirements = extractJobRequirements(smartCaptureText);
    setApplicationDraft({
      ...emptyApplication,
      ...
      requirements: detectedRequirements.join("\n"),
      requirementEvidence: detectedRequirements.map((requirement) => ({ requirement, evidence: "" })),
  ...
  function openInterviewSupport(application: Application) {
    ...
    const evidenceContext = applicationEvidenceContext(application, language);
    setInterviewPrepDraft(language === "he"
      ? `הכנה לראיון · ${application.role} ב־${application.company}\n\nדרישות וראיות\n${evidenceContext}\n\nסיפורי STAR להכנה\n1. סיפור שמוכיח את ההתאמה החזקה ביותר:\nמצב:\nמשימה:\nפעולה:\nתוצאה:\n\n2. סיפור שמחזק דרישה ללא ראיה מספקת:\nמצב:\nמשימה:\nפעולה:\nתוצאה:\n\nשאלות למראיינים\n• כיצד נראית הצלחה בתפקיד לאחר 90 יום?\n• מהו האתגר המרכזי שהאדם שייכנס לתפקיד יצטרך לפתור?\n\nפערים שכדאי להסביר בכנות\nעברו על כל שורה המסומנת כפער והכינו תשובה קצרה שמדגישה יכולת למידה או ניסיון משיק.`
      : `INTERVIEW BRIEF · ${application.role} at ${application.company}\n\nREQUIREMENTS & EVIDENCE\n${evidenceContext}\n\nSTAR STORIES TO PREPARE\n1. Story that proves your strongest match:\nSituation:\nTask:\nAction:\nResult:\n\n2. Story that strengthens a requirement with limited evidence:\nSituation:\nTask:\nAction:\nResult:\n\nQUESTIONS FOR THE INTERVIEWERS\n• What would success look like in the first 90 days?\n• What is the most important problem this hire should solve?\n\nGAPS TO EXPLAIN HONESTLY\nReview every item marked as a gap and prepare a concise answer grounded in adjacent experience or learning ability.`);
    setExpandedTools((current) => ({ ...current, interview: true, offer: false, cv: false, studio: false, social: false }));
    navigateToSection("interview-prep");
  ...
  function openCvSupport(application: Application) {
    setWorkspaceApplicationId(null);
    const evidenceContext = applicationEvidenceContext(application, language);
    setRewriteDraft(language === "he"
      ? `התאמת קורות חיים · ${application.role} ב־${application.company}\n\nמטרת העריכה\nהתאימו את הניסוח לדרישות המשרה תוך שימוש רק בראיות אמיתיות. אין להמציא ניסיון, מספרים או הישגים.\n\nדרישות וראיות זמינות\n${evidenceContext}\n\nתקציר מקצועי מותאם\n[כתבו 3–4 שורות המדגישות את הראיות הרלוונטיות ביותר]\n\nניסיון והישגים שכדאי להבליט\n• [הפכו כל ראיה לבולט תמציתי עם פעולה ותוצאה]\n\nבדיקה סופית\n• כל טענה נתמכת בניסיון אמיתי.\n• מילות מפתח משולבות בטבעיות.\n• פערים אינם מוצגים כניסיון קיים.`
      : `CV TAILORING · ${application.role} at ${application.company}\n\nEDITING GOAL\nAlign the CV with the role requirements using only real evidence. Do not invent experience, numbers, or outcomes.\n\nAVAILABLE REQUIREMENTS & EVIDENCE\n${evidenceContext}\n\nTAILORED PROFESSIONAL PROFILE\n[Write 3–4 lines emphasizing the strongest relevant evidence]\n\nEXPERIENCE & IMPACT TO HIGHLIGHT\n• [Turn each proof point into a concise action-and-result bullet]\n\nFINAL CHECK\n• Every claim is grounded in real experience.\n• Keywords are used naturally.\n• Gaps are not presented as existing experience.`);
    setExpandedTools((current) => ({ ...current, cv: true, interview: false, offer: false, studio: false, social: false }));
    navigateToSection("cv-lab");
    setNotice(language === "he" ? "דרישות וראיות מהמועמדות הועברו למעבדת קורות החיים." : "Role requirements and evidence were added to the CV Lab.");
  }

  function openOfferDecision(application: Application) {
    setWorkspaceApplicationId(null);
    const readiness = offerReadiness(application, language);
    const offer = application.offerDetails;
    const expectationLine = application.salary.trim()
      ? `${application.salaryCurrency} ${application.salary.trim()}`
      : (language === "he" ? "לא נשמר" : "Not saved");
    const offerSalaryLine = offer.baseSalary.trim() ? `${offer.currency} ${offer.baseSalary.trim()}` : (language === "he" ? "לא נשמר" : "Not saved");
    const negotiationLines = offer.negotiationTopics
      .filter((item) => item.status !== "Not discussed" || item.target.trim() || item.outcome.trim())
      .map((item) => language === "he"
        ? `• ${negotiationTopicLabel(item.topic, language)} — ${negotiationStatusLabel(item.status, language)}\n  מקורי: ${item.original || "לא נשמר"} | יעד: ${item.target || "לא נשמר"} | תוצאה: ${item.outcome || "טרם נקבעה"}`
        : `• ${negotiationTopicLabel(item.topic, language)} — ${negotiationStatusLabel(item.status, language)}\n  Original: ${item.original || "Not saved"} | Target: ${item.target || "Not saved"} | Outcome: ${item.outcome || "Not decided"}`)
      .join("\n");
    setOfferDecisionDraft(language === "he"
      ? `מסמך החלטה על הצעה · ${application.role} ב־${application.company}\n\nתנאי הצעה מאומתים\n• שכר בסיס: ${offerSalaryLine}\n• בונוס: ${offer.bonus || "לא נשמר"}\n• הון / מניות: ${offer.equity || "לא נשמר"}\n• הטבות: ${offer.benefits || "לא נשמר"}\n• תפקיד או דרגה: ${offer.titleLevel || "לא נשמר"}\n• תאריך התחלה: ${offer.startDate ? formatDate(offer.startDate) : "לא נשמר"}\n• מועד החלטה רשמי: ${offer.responseDeadline ? formatDate(offer.responseDeadline) : "לא נשמר"}\n• מודל עבודה מאומת: ${offer.workModel || "לא נשמר"}\n\nציפיות והקשר — לא תנאי הצעה\n• ציפיית שכר: ${expectationLine}\n• טווח החברה: ${application.budgetRange || "לא נשמר"}\n\nמעקב משא ומתן\n${negotiationLines || "• עדיין לא תועדו נושאים למשא ומתן."}\n\nהחלטה\n• סטטוס: ${offerDecisionLabel(offer.decisionOutcome, language)}\n• תאריך: ${offer.decisionDate ? formatDate(offer.decisionDate) : "לא נשמר"}\n• נימוק: ${offer.decisionReason || "לא נשמר"}\n\nמידע שחסר לפני החלטה\n${readiness.missingLabels.length ? readiness.missingLabels.map((item) => `• ${item}`).join("\n") : "• הפרטים המרכזיים נשמרו. עדיין יש לאמת אותם מול ההצעה הכתובה."}\n\nמה חשוב לי בהחלטה\n1. התפתחות מקצועית:\n2. מנהל/ת וצוות:\n3. תוכן התפקיד והשפעה:\n4. שכר והטבות:\n5. גמישות ואיזון:\n\nשאלות לשיחה או למשא ומתן\n• כיצד מוגדרת הצלחה ב־90 הימים הראשונים?\n• מה כולל מבנה השכר וההטבות המלא?\n• מהו טווח הגמישות לגבי שכר, דרגה, תאריך התחלה ומודל עבודה?\n• האם אפשר לקבל את כל התנאים והמועד להחלטה בכתב?\n\nבדיקת מציאות\n• יש לאמת את כל התנאים מול ההצעה הרשמית לפני החלטה.\n• אומת לאחרונה: ${offer.verifiedAt ? formatDate(offer.verifiedAt) : "לא תועד אימות"}\n\nהערות קיימות\n${application.notes || "אין עדיין הערות."}`
      : `OFFER DECISION BRIEF · ${application.role} at ${application.company}\n\nVERIFIED OFFER TERMS\n• Base salary: ${offerSalaryLine}\n• Bonus: ${offer.bonus || "Not saved"}\n• Equity: ${offer.equity || "Not saved"}\n• Benefits: ${offer.benefits || "Not saved"}\n• Title or level: ${offer.titleLevel || "Not saved"}\n• Start date: ${offer.startDate ? formatDate(offer.startDate) : "Not saved"}\n• Official response deadline: ${offer.responseDeadline ? formatDate(offer.responseDeadline) : "Not saved"}\n• Confirmed work model: ${offer.workModel || "Not saved"}\n\nEXPECTATIONS & CONTEXT — NOT OFFER TERMS\n• Salary expectation: ${expectationLine}\n• Company range: ${application.budgetRange || "Not saved"}\n\nNEGOTIATION TRACKER\n${negotiationLines || "• No negotiation topics tracked yet."}\n\nDECISION\n• Status: ${offerDecisionLabel(offer.decisionOutcome, language)}\n• Date: ${offer.decisionDate ? formatDate(offer.decisionDate) : "Not saved"}\n• Reason: ${offer.decisionReason || "Not saved"}\n\nMISSING BEFORE A DECISION\n${readiness.missingLabels.length ? readiness.missingLabels.map((item) => `• ${item}`).join("\n") : "• The core details are saved. Verify them against the written offer."}\n\nWHAT MATTERS TO ME\n1. Growth and learning:\n2. Manager and team:\n3. Scope and impact:\n4. Compensation and benefits:\n5. Flexibility and balance:\n\nQUESTIONS FOR THE OFFER OR NEGOTIATION CALL\n• What would success look like in the first 90 days?\n• What is included in the full compensation and benefits package?\n• Where is there flexibility on compensation, level, start date, or work model?\n• Can I receive all terms and the decision deadline in writing?\n\nREALITY CHECK\n• Verify every term against the official written offer before deciding.\n• Last verified: ${offer.verifiedAt ? formatDate(offer.verifiedAt) : "No verification recorded"}\n\nSAVED NOTES\n${application.notes || "No notes saved yet."}`);
    setExpandedTools((current) => ({ ...current, offer: true, interview: false, cv: false, studio: false, social: false }));
    navigateToSection("offer-decision");
    setNotice(language === "he" ? "נוצר מסמך החלטה המבוסס רק על פרטי ההצעה ששמרתם." : "Decision brief created only from the offer details you saved.");
  }
  ...
  function saveApplication(event: FormEvent<HTMLFormElement>) {
    ...
    if (savedApplication.status === "Offer" && savedApplication.offerDetails.responseDeadline && !savedApplication.nextStepDue) {
      savedApplication.nextStepDue = savedApplication.offerDetails.responseDeadline;
    }
  ...
  function deleteApplication(application: Application) {
    const confirmed = window.confirm(language === "he" ? `למחוק את המועמדות לתפקיד ${application.role} ב־${application.company}?` : `Delete the ${application.role} application at ${application.company}?`);
    if (confirmed) {
      setApplications((items) => items.filter((item) => item.id !== application.id));
      setNotice(language === "he" ? "המועמדות נמחקה." : "Application deleted.");
    }
  }
  ...
  function openOutreachForApplication(application: Application) {
    ...
    setMessageProfile({
      ...
      intent: "Follow up after applying",
  ...
  function executeTodayPrimaryAction() {
    const action = nextBestActions[0];
    if (!applications.length && !contacts.length) { openNewApplication(); return; }
    if (primaryActionApplication && action?.id.startsWith("app-plan-")) { openEditApplication(primaryActionApplication); return; }
    if (action) { executeNextBestAction(action); return; }
    navigateToSection(homepageAction.target);
  }

  function executeAttentionAction(action: NextBestAction) {
    const application = action.applicationId ? applications.find((item) => item.id === action.applicationId) || null : null;
    if (application && action.id.startsWith("app-plan-")) { openEditApplication(application); return; }
    executeNextBestAction(action);
  }

  function viewAllAttention() {
    const attentionActions = nextBestActions.slice(1);
    const contactsOnly = attentionActions.length > 0 && attentionActions.every((action) => action.contactId);
    if (contactsOnly) { switchView("networking"); return; }
  ...
  return (
    <main className={`carvio-shell min-h-screen px-4 py-6 text-slate-100 sm:px-6 sm:py-8 lg:px-8 ${theme === "light" ? "carvio-light" : theme === "ocean" ? "carvio-ocean" : theme === "plum" ? "carvio-plum" : ""}`} dir={language === "he" ? "rtl" : "ltr"}>
      <div className="calm-content mx-auto flex max-w-7xl flex-col gap-8">
        ...
        <header className={`calm-view dashboard-v2 ${activeView !== "home" ? "calm-view-hidden" : ""}`} id="dashboard">
          <div className="carvio-hero-top">
            <div className="dashboard-v2-greeting">
              <p>{language === "he" ? "בוקר טוב" : "Good morning"}{userProfile.name ? `, ${userProfile.name}` : ""}</p>
              <span>{language === "he" ? "זה מה שחשוב היום." : "Here’s what matters today."}</span>
            </div>
          ...
          <section className="dashboard-v2-today" aria-labelledby="dashboard-v2-today-title">
            <div className="dashboard-v2-today-layout">
              <div className="dashboard-v2-today-content">
                <p className="dashboard-v2-label"><Zap className="h-4 w-4" />{language === "he" ? "היום" : "Today"}</p>
                <div className="dashboard-v2-today-copy">
                  <span>{nextBestActions[0]?.kind || todayFocus.eyebrow}</span>
                  <h1 id="dashboard-v2-today-title">{todayActionPresentation.headline}</h1>
                  <p>{todayActionPresentation.reason}</p>
                </div>
                <div className="dashboard-v2-today-actions">
                  <button className="dashboard-v2-primary" onClick={executeTodayPrimaryAction} type="button">{todayActionPresentation.cta}<ArrowUpRight className="h-4 w-4" /></button>
                  {primaryActionApplication && todayActionPresentation.canComplete && <button onClick={() => completeApplicationAction(primaryActionApplication)} type="button"><CheckCircle2 className="h-4 w-4" />{language === "he" ? "בוצע" : "Done"}</button>}
                  {(primaryActionApplication || primaryActionContact) && <div className="dashboard-v2-overflow" ref={todayOverflowRef}>
                    <button aria-expanded={showTodayOverflow} aria-haspopup="menu" aria-label={language === "he" ? "פעולות נוספות להמלצה" : "More recommendation actions"} className="dashboard-v2-more" onClick={() => setShowTodayOverflow((current) => !current)} type="button">•••</button>
                    {showTodayOverflow && <div className="dashboard-v2-overflow-menu" role="menu">
                      {primaryActionApplication && nextBestActions[0]?.intent !== "recover" && <button onClick={() => { snoozeApplication(primaryActionApplication, 2); setShowTodayOverflow(false); }} role="menuitem" type="button"><Clock3 className="h-4 w-4" />{language === "he" ? "דחייה ביומיים" : "Snooze 2 days"}</button>}
                      {primaryActionApplication && nextBestActions[0]?.intent !== "recover" && <button onClick={() => { snoozeApplication(primaryActionApplication, 7); setShowTodayOverflow(false); }} role="menuitem" type="button"><Clock3 className="h-4 w-4" />{language === "he" ? "דחייה בשבוע" : "Snooze 1 week"}</button>}
                      {primaryActionApplication && <button onClick={() => { setWorkspaceApplicationId(primaryActionApplication.id); setShowTodayOverflow(false); }} role="menuitem" type="button"><BriefcaseBusiness className="h-4 w-4" />{language === "he" ? "פתיחת סביבת המועמדות" : "Open workspace"}</button>}
                      {primaryActionApplication && <button onClick={() => { openEditApplication(primaryActionApplication); setShowTodayOverflow(false); }} role="menuitem" type="button"><Pencil className="h-4 w-4" />{language === "he" ? "עריכת המועמדות" : "Edit application"}</button>}
                      {primaryActionContact && <button onClick={() => { setContactQuery(primaryActionContact.name); switchView("networking"); setShowTodayOverflow(false); }} role="menuitem" type="button"><MessagesSquare className="h-4 w-4" />{language === "he" ? "פתיחת השיחה" : "Open conversation"}</button>}
                      {primaryActionContact && <button onClick={() => { openEditContact(primaryActionContact); setShowTodayOverflow(false); }} role="menuitem" type="button"><Pencil className="h-4 w-4" />{language === "he" ? "עריכת איש הקשר" : "Edit contact"}</button>}
                    </div>}
                  </div>}
                </div>
                {nextBestActions.length > 1 && <button className="dashboard-v2-attention-link" onClick={() => document.getElementById("dashboard-attention")?.scrollIntoView({ behavior: "smooth", block: "center" })} type="button">{language === "he" ? `${nextBestActions.length - 1} פריטים נוספים דורשים תשומת לב` : `${nextBestActions.length - 1} other ${nextBestActions.length === 2 ? "item needs" : "items need"} attention`}<ArrowDown className="h-4 w-4" /></button>}
              </div>
              <figure aria-hidden="true" className="dashboard-v2-today-visual"><Image alt="" fill priority sizes="(max-width: 420px) 0px, (max-width: 700px) calc(100vw - 3rem), 320px" src="/carvio-next-action-recovery-v1.jpg" /></figure>
            </div>
          </section>

          <section className="dashboard-v2-section dashboard-v2-attention" id="dashboard-attention" aria-labelledby="dashboard-attention-title">
            <div className="dashboard-v2-section-heading"><div><p className="dashboard-v2-label"><CircleAlert className="h-4 w-4" />{language === "he" ? "דורש תשומת לב" : "Needs attention"}</p><h2 id="dashboard-attention-title">{language === "he" ? "דברים שלא כדאי שייפלו בין הכיסאות" : "Keep these from falling through the cracks"}</h2></div>{attentionPresentations.length > 3 && <button onClick={viewAllAttention} type="button">{language === "he" ? `לכל ${attentionPresentations.length} הפריטים` : `View all ${attentionPresentations.length}`}<ArrowUpRight className="h-4 w-4" /></button>}</div>
            {attentionPresentations.length > 0 ? <div className="dashboard-v2-attention-list">{attentionPresentations.slice(0, 3).map((item) => {
              const AttentionIcon = item.icon === "meeting" ? CalendarClock : item.icon === "contact" ? Users2 : item.icon === "plan" ? Target : item.icon === "recover" ? HeartHandshake : item.icon === "overdue" ? Clock3 : BriefcaseBusiness;
              return <article className="dashboard-v2-attention-row" key={item.action.id}>
                <span className={`dashboard-v2-attention-icon dashboard-v2-attention-icon-${item.tone}`}><AttentionIcon className="h-4 w-4" /></span>
                <span className="dashboard-v2-attention-copy"><strong>{item.headline}</strong><em>{item.reason}</em></span>
                <span className="dashboard-v2-attention-controls">{item.urgency && <small className={`dashboard-v2-urgency dashboard-v2-urgency-${item.tone}`}>{item.urgency}</small>}<button onClick={() => executeAttentionAction(item.action)} type="button">{item.cta}<ChevronRight className="h-3.5 w-3.5" /></button></span>
              </article>;
            })}</div> : <div className="dashboard-v2-empty"><CheckCircle2 className="h-5 w-5" /><span><strong>{language === "he" ? "שום דבר נוסף לא דורש תשומת לב" : "Nothing else needs attention"}</strong><small>{language === "he" ? "הכול ברור לעת עתה." : "You’re clear for now."}</small></span></div>}
          </section>

          <section className={`dashboard-v2-section dashboard-v2-progress ${searchProgress.isEmpty ? "dashboard-v2-progress-empty" : ""}`} aria-labelledby="dashboard-progress-title">
            <div className="dashboard-v2-section-heading"><div><p className="dashboard-v2-label"><TrendingUp className="h-4 w-4" />{language === "he" ? "התקדמות בחיפוש" : "Search progress"}</p><h2 id="dashboard-progress-title">{searchProgress.isEmpty ? (language === "he" ? "ההתקדמות מתחילה כאן" : "Progress starts here") : (language === "he" ? "החיפוש שלך בתנועה" : "Your search is moving")}</h2></div><button onClick={() => switchView("more")} type="button">{language === "he" ? "לצפייה בתובנות" : "View insights"}<ArrowUpRight className="h-4 w-4" /></button></div>
            <div className="dashboard-v2-progress-flow" aria-label={language === "he" ? "מסלול ההתקדמות בחיפוש" : "Job search progress flow"}>
              <span><strong>{searchProgress.activeOpportunities}</strong><small>{language === "he" ? "הזדמנויות פעילות" : "active opportunities"}</small></span><ArrowRight className="h-4 w-4" />
              <span><strong>{searchProgress.conversations}</strong><small>{language === "he" ? "שיחות שתועדו" : "conversations"}</small></span><ArrowRight className="h-4 w-4" />
              <span><strong>{searchProgress.interviews}</strong><small>{language === "he" ? "הגיעו לראיון" : "reached interview"}</small></span>
            </div>
            <p className="dashboard-v2-weekly"><TrendingUp className="h-4 w-4" />{searchProgress.statement}</p>
          </section>
        </header>
                  ...
                  {visibleApplications.map((application) => {
                    ...
                    return <article className="mobile-record-card" key={application.id}>
                      ...
                      <div className="mobile-record-actions">
                        <button className="mobile-workspace-trigger" onClick={() => setWorkspaceApplicationId(application.id)} type="button"><BriefcaseBusiness className="h-4 w-4" />{language === "he" ? "סביבת עבודה" : "Workspace"}</button>
        ...
        <section className={`calm-view social-studio-panel ${activeView !== "tools" ? "calm-view-hidden" : ""}`} id="social-studio">
          <div className="section-heading tool-card-heading relative">
            ...
            <div className="tool-card-actions"><button aria-expanded={expandedTools.social} className="secondary-button" onClick={() => setExpandedTools((current) => ({ ...current, social: !current.social, studio: false, cv: false, interview: false, offer: false }))} type="button">{expandedTools.social ? (language === "he" ? "סגירת הסטודיו" : "Close studio") : (language === "he" ? "פתיחת הסטודיו" : "Open studio")}<ChevronDown className={`h-4 w-4 transition ${expandedTools.social ? "rotate-180" : ""}`} /></button></div>
          </div>
        ...
        <section className={`calm-view panel interview-prep-panel ${activeView !== "tools" ? "calm-view-hidden" : ""}`} id="interview-prep">
          <div className="section-heading tool-card-heading">
            <div className="tool-card-copy"><p className="eyebrow flex items-center gap-2 text-amber-300"><Target className="h-4 w-4" />{language === "he" ? "הכנה ממוקדת־ראיות" : "Evidence-led preparation"}</p><h2 className="section-title">{language === "he" ? "הגיעו לראיון עם סיפורים שמוכיחים התאמה" : "Enter the interview with stories that prove your fit"}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">{language === "he" ? "Carvio מחבר בין דרישות המשרה, ראיות אמיתיות ופערים שכדאי להסביר—ללא המצאת ניסיון." : "Carvio connects role requirements, real proof points, and gaps worth explaining—without inventing experience."}</p></div>
            <figure className="tool-card-illustration"><Image alt={language === "he" ? "איור של הכנה לראיון מקצועי" : "Illustration of professional interview preparation"} fill sizes="(max-width: 767px) 92px, 176px" src="/carvio-interview-preparation-v1.jpg" /></figure>
            <div className="tool-card-actions"><button aria-expanded={expandedTools.interview} className="secondary-button" onClick={() => setExpandedTools((current) => ({ ...current, interview: !current.interview, cv: false, offer: false, studio: false, social: false }))} type="button">{expandedTools.interview ? (language === "he" ? "סגירת ההכנה" : "Close preparation") : (language === "he" ? "פתיחת הכנת ראיון" : "Open interview preparation")}<ChevronDown className={`h-4 w-4 transition ${expandedTools.interview ? "rotate-180" : ""}`} /></button></div>
          </div>
          {expandedTools.interview && <div className="interview-prep-workspace"><div><h3>{language === "he" ? "מסמך ההכנה שלכם" : "Your preparation brief"}</h3><p>{language === "he" ? "ערכו את הסיפורים והשאלות עד שהם מדויקים, קצרים ואמיתיים." : "Edit the stories and questions until they are specific, concise, and true."}</p></div>{interviewPrepDraft ? <><textarea aria-label={language === "he" ? "מסמך הכנה לראיון" : "Interview preparation brief"} className="form-control" onChange={(event) => setInterviewPrepDraft(event.target.value)} value={interviewPrepDraft} /><div className="workspace-context-actions"><button className="secondary-button" onClick={() => { void navigator.clipboard.writeText(interviewPrepDraft); setNotice(language === "he" ? "מסמך ההכנה הועתק." : "Preparation brief copied."); }} type="button"><Copy className="h-4 w-4" />{language === "he" ? "העתקה" : "Copy brief"}</button></div></> : <div className="interview-prep-empty"><Target className="h-7 w-7" /><strong>{language === "he" ? "פתחו מועמדות כדי להתחיל" : "Open an application to begin"}</strong><p>{language === "he" ? "מתוך ה־Workspace בחרו ‘הכנת ראיון ו־STAR’." : "Choose ‘Interview & STAR preparation’ from an application workspace."}</p></div>}</div>}
        </section>

        <section className={`calm-view panel interview-prep-panel offer-decision-panel ${activeView !== "tools" ? "calm-view-hidden" : ""}`} id="offer-decision">
          <div className="section-heading tool-card-heading">
            <div className="tool-card-copy"><p className="eyebrow flex items-center gap-2 text-emerald-300"><Scale className="h-4 w-4" />{language === "he" ? "החלטה מבוססת־מידע" : "Grounded offer decision"}</p><h2 className="section-title">{language === "he" ? "הבינו את ההצעה לפני שאתם משיבים" : "Understand the offer before you respond"}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">{language === "he" ? "Carvio מרכז את מה שכבר נשמר, מציף פרטים חסרים ומכין שאלות לשיחה—בלי להמציא תנאים." : "Carvio brings together what you saved, surfaces missing terms, and prepares questions for the conversation—without inventing details."}</p></div>
            <div className="offer-decision-visual" aria-hidden="true"><Scale className="h-8 w-8" /><span>{language === "he" ? "בהירות לפני החלטה" : "Clarity before commitment"}</span></div>
            <div className="tool-card-actions"><button aria-expanded={expandedTools.offer} className="secondary-button" onClick={() => setExpandedTools((current) => ({ ...current, offer: !current.offer, interview: false, cv: false, studio: false, social: false }))} type="button">{expandedTools.offer ? (language === "he" ? "סגירת המסמך" : "Close decision brief") : (language === "he" ? "פתיחת מסמך החלטה" : "Open decision brief")}<ChevronDown className={`h-4 w-4 transition ${expandedTools.offer ? "rotate-180" : ""}`} /></button></div>
          </div>
          {expandedTools.offer && <div className="interview-prep-workspace offer-decision-workspace"><div><h3>{language === "he" ? "מסמך ההחלטה שלכם" : "Your offer decision brief"}</h3><p>{language === "he" ? "השלימו את החסר, כתבו מה חשוב לכם ואמתו כל תנאי מול ההצעה הרשמית." : "Fill the gaps, capture what matters to you, and verify every term against the official offer."}</p></div>{offerDecisionDraft ? <><textarea aria-label={language === "he" ? "מסמך החלטה על הצעה" : "Offer decision brief"} className="form-control" onChange={(event) => setOfferDecisionDraft(event.target.value)} value={offerDecisionDraft} /><div className="workspace-context-actions"><button className="secondary-button" onClick={() => { void navigator.clipboard.writeText(offerDecisionDraft); setNotice(language === "he" ? "מסמך ההחלטה הועתק." : "Offer decision brief copied."); }} type="button"><Copy className="h-4 w-4" />{language === "he" ? "העתקה" : "Copy brief"}</button></div></> : <div className="interview-prep-empty"><Scale className="h-7 w-7" /><strong>{language === "he" ? "פתחו הצעה כדי להתחיל" : "Open an offer to begin"}</strong><p>{language === "he" ? "מתוך סביבת מועמדות בשלב הצעה, בחרו ‘בדיקת ההצעה’." : "From an application at Offer stage, choose ‘Review offer’."}</p></div>}</div>}
        </section>
        ...
        <Modal title={language === "he" ? "סביבת המועמדות" : "Application workspace"} description={language === "he" ? "הבינו מה קורה, למה זה חשוב ומה כדאי לעשות עכשיו." : "See what is happening, why it matters, and what to do next."} closeLabel={language === "he" ? "סגירת החלון" : "Close modal"} onClose={() => setWorkspaceApplicationId(null)} wide>
          <div className="application-workspace application-workspace-v2">
            ...
            <div className="workspace-grid">
              {workspaceOfferReadiness && <section className="workspace-offer-readiness"><div className="workspace-section-heading"><div><h4><Scale className="h-4 w-4" />{language === "he" ? "מוכנות להחלטה" : "Offer readiness"}</h4><small>{language === "he" ? "הציון משקף שלמות ותזמון בלבד — לא את איכות ההצעה." : "Readiness reflects completeness and timing—not offer quality."}</small></div><strong className="workspace-evidence-score">{workspaceOfferReadiness.score}%</strong></div><div className="offer-readiness-state"><strong>{workspaceOfferReadiness.state}</strong>{workspaceApplication.offerDetails.responseDeadline && <span>{language === "he" ? "מועד החלטה" : "Decision deadline"}: {formatDate(workspaceApplication.offerDetails.responseDeadline)}</span>}</div>{workspaceOfferReadiness.missingLabels.length > 0 && <p className="workspace-snapshot-missing"><CircleAlert className="h-4 w-4" />{language === "he" ? `חסר: ${workspaceOfferReadiness.missingLabels.slice(0, 3).join(", ")}` : `Missing: ${workspaceOfferReadiness.missingLabels.slice(0, 3).join(", ")}`}</p>}<button className="text-button workspace-offer-action" onClick={() => { setWorkspaceApplicationId(null); openEditApplication(workspaceApplication); }} type="button"><Pencil className="h-4 w-4" />{workspaceOfferReadiness.score ? (language === "he" ? "עריכת תנאי ההצעה" : "Edit offer terms") : (language === "he" ? "הוספת תנאי הצעה" : "Add offer terms")}</button></section>}
              {workspaceNegotiation && <section className="workspace-negotiation"><div className="workspace-section-heading"><div><h4><MessagesSquare className="h-4 w-4" />{language === "he" ? "משא ומתן והחלטה" : "Negotiation & decision"}</h4><small>{language === "he" ? "תיעוד עובדתי של מה שביקשתם ומה שסוכם — ללא המלצה אוטומטית." : "A factual record of what you requested and what was agreed—without an automated recommendation."}</small></div><span className={`decision-pill decision-${workspaceApplication.offerDetails.decisionOutcome.toLowerCase().replaceAll(" ", "-")}`}>{offerDecisionLabel(workspaceApplication.offerDetails.decisionOutcome, language)}</span></div><div className="negotiation-summary"><div><strong>{workspaceNegotiation.tracked}</strong><span>{language === "he" ? "נושאים במעקב" : "topics tracked"}</span></div><div><strong>{workspaceNegotiation.active}</strong><span>{language === "he" ? "בדיון" : "in discussion"}</span></div><div><strong>{workspaceNegotiation.resolved}</strong><span>{language === "he" ? "הוכרעו" : "resolved"}</span></div></div>{workspaceNegotiation.tracked > 0 ? <div className="negotiation-topic-list">{workspaceApplication.offerDetails.negotiationTopics.filter((item) => item.status !== "Not discussed" || item.target.trim() || item.outcome.trim()).map((item) => <article key={item.id}><div><strong>{negotiationTopicLabel(item.topic, language)}</strong><span className={`negotiation-status status-${item.status.toLowerCase().replaceAll(" ", "-")}`}>{negotiationStatusLabel(item.status, language)}</span></div><p>{item.original && <span>{language === "he" ? "מקורי" : "Original"}: {item.original}</span>}{item.target && <span>{language === "he" ? "יעד" : "Target"}: {item.target}</span>}{item.outcome && <span>{language === "he" ? "תוצאה" : "Outcome"}: {item.outcome}</span>}</p></article>)}</div> : <p className="workspace-empty">{language === "he" ? "עדיין לא תועדו נושאים למשא ומתן." : "No negotiation topics are tracked yet."}</p>}{workspaceApplication.offerDetails.decisionReason && <p className="decision-reason"><strong>{language === "he" ? "נימוק ההחלטה" : "Decision rationale"}</strong>{workspaceApplication.offerDetails.decisionReason}</p>}<button className="text-button workspace-offer-action" onClick={() => { setWorkspaceApplicationId(null); openEditApplication(workspaceApplication); }} type="button"><Pencil className="h-4 w-4" />{language === "he" ? "עדכון המשא ומתן וההחלטה" : "Update negotiation and decision"}</button></section>}
              ...
              {workspaceApplication.requirementEvidence.length > 0 && <section className="workspace-evidence-map"><div className="workspace-section-heading"><div><h4><Target className="h-4 w-4" />{language === "he" ? "דרישות מול ראיות" : "Requirements to evidence"}</h4><small>{language === "he" ? `${workspaceApplication.requirementEvidence.filter((item) => item.evidence.trim()).length} מתוך ${workspaceApplication.requirementEvidence.length} דרישות מגובות בראיה.` : `${workspaceApplication.requirementEvidence.filter((item) => item.evidence.trim()).length} of ${workspaceApplication.requirementEvidence.length} requirements backed by evidence.`}</small></div><strong className="workspace-evidence-score">{Math.round((workspaceApplication.requirementEvidence.filter((item) => item.evidence.trim()).length / workspaceApplication.requirementEvidence.length) * 100)}%</strong></div><div className="workspace-evidence-list">{workspaceApplication.requirementEvidence.slice(0, 8).map((item, index) => <article className={item.evidence.trim() ? "has-evidence" : "needs-evidence"} key={`${item.requirement}-${index}`}><span>{item.evidence.trim() ? <CheckCircle2 className="h-4 w-4" /> : <CircleAlert className="h-4 w-4" />}</span><div><strong>{item.requirement}</strong><p>{item.evidence.trim() || (language === "he" ? "עדיין חסרה דוגמה שמוכיחה התאמה." : "A proof point is still missing.")}</p></div></article>)}</div><button className="text-button" onClick={() => { setWorkspaceApplicationId(null); openEditApplication(workspaceApplication); }} type="button"><Pencil className="h-4 w-4" />{language === "he" ? "הוספת ראיות מהניסיון" : "Add evidence from experience"}</button></section>}
              ...
              <section><div className="workspace-section-heading"><div><h4>{language === "he" ? "הכנה, הודעות וקורות חיים" : "Preparation, messages & CV"}</h4><small>{language === "he" ? "תבניות מקומיות ושקופות המבוססות רק על הנתונים ששמרתם." : "Transparent local templates based only on the context you saved."}</small></div><figure className="workflow-illustration workflow-illustration-tiny"><Image alt={language === "he" ? "איור של הכנה רגועה לראיון" : "Illustration of calm interview preparation"} fill sizes="96px" src="/carvio-interview-preparation-v1.jpg" /></figure></div>{resumes.length ? resumes.slice(0, 3).map((resume) => <div className="workspace-file" key={resume.id}><FileText className="h-4 w-4" /><span><strong>{resume.name}</strong><small>{new Date(resume.addedAt).toLocaleDateString()}</small></span></div>) : <p className="workspace-empty">{language === "he" ? "טרם נוספה גרסת קורות חיים." : "No CV version added yet."}</p>}<div className="workspace-context-actions">{workspaceApplication.status === "Offer" && <button className="text-button workspace-offer-action" onClick={() => openOfferDecision(workspaceApplication)} type="button"><Scale className="h-4 w-4" />{language === "he" ? "בדיקת ההצעה" : "Review offer"}</button>}<button className="text-button" onClick={() => openInterviewSupport(workspaceApplication)} type="button"><Target className="h-4 w-4" />{language === "he" ? "הכנת ראיון ו־STAR" : "Interview & STAR preparation"}</button><button className="text-button" onClick={() => openCvSupport(workspaceApplication)} type="button"><FileCheck2 className="h-4 w-4" />{language === "he" ? "התאמת קורות חיים לדרישות" : "Tailor CV to requirements"}</button></div></section>
      ...
      {showApplicationModal && (
        <Modal title={language === "he" ? (editingApplicationId ? "עריכת מועמדות" : "הוספת מועמדות") : (editingApplicationId ? "Edit application" : "Add application")} description={language === "he" ? "תעדו את ההזדמנות, מצב התהליך, לוחות הזמנים והצעד הבא." : "Capture the opportunity, its signal, timing, and your next move."} closeLabel={language === "he" ? "סגירת החלון" : "Close modal"} onClose={() => setShowApplicationModal(false)}>
          <form className="application-form-compact space-y-4" onSubmit={saveApplication}>
            ...
            <section className="application-snapshot-fields"><div><p><FileText className="h-4 w-4" />{language === "he" ? "צילום מצב של המשרה" : "Job snapshot"}</p><small>{language === "he" ? "שמרו את התוכן החשוב גם אם המודעה המקורית תוסר." : "Keep the important context even if the original listing is removed."}</small></div><Field label={language === "he" ? "תיאור המשרה" : "Job description"}><textarea className="form-control min-h-32 resize-y" onChange={(e) => setApplicationDraft({ ...applicationDraft, jobDescription: e.target.value })} placeholder={language === "he" ? "תחומי אחריות, הקשר על הצוות ופרטי התפקיד" : "Responsibilities, team context, and role details"} value={applicationDraft.jobDescription} /></Field><Field label={language === "he" ? "דרישות מרכזיות" : "Key requirements"}><textarea className="form-control min-h-24 resize-y" onChange={(e) => { const requirements = e.target.value; setApplicationDraft((current) => ({ ...current, requirements, requirementEvidence: syncRequirementEvidence(requirements, current.requirementEvidence) })); }} placeholder={language === "he" ? "דרישה אחת בכל שורה" : "One requirement per line"} value={applicationDraft.requirements} /></Field>{applicationDraft.requirementEvidence.length > 0 && <div className="requirement-evidence-editor"><div><p><Target className="h-4 w-4" />{language === "he" ? "מפת ראיות להתאמה" : "Fit evidence map"}</p><small>{language === "he" ? "לכל דרישה, הוסיפו דוגמה קצרה שמוכיחה התאמה. השאירו ריק אם אין עדיין ראיה." : "Add one concise proof point for each requirement. Leave it blank when evidence is still missing."}</small></div>{applicationDraft.requirementEvidence.map((item, index) => <label key={`${item.requirement}-${index}`}><span>{item.requirement}</span><textarea className="form-control" onChange={(event) => setApplicationDraft((current) => ({ ...current, requirementEvidence: current.requirementEvidence.map((entry, entryIndex) => entryIndex === index ? { ...entry, evidence: event.target.value } : entry) }))} placeholder={language === "he" ? "דוגמה, הישג או תוצאה מהניסיון שלכם" : "Example, achievement, or result from your experience"} value={item.evidence} /></label>)}</div>}</section>
            ...
            {applicationDraft.status === "Offer" && <section className="offer-terms-editor"><div><p><Scale className="h-4 w-4" />{language === "he" ? "תנאי ההצעה המאומתים" : "Verified offer terms"}</p><small>{language === "he" ? "העתיקו רק פרטים מההצעה הכתובה או כאלה שאומתו מול החברה. כל השדות אופציונליים." : "Enter only terms from the written offer or details confirmed by the company. Every field is optional."}</small></div><div className="grid gap-4 sm:grid-cols-[110px_1fr_1fr]"><Field label={language === "he" ? "מטבע ההצעה" : "Offer currency"}><select className="form-control" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, currency: event.target.value as OfferDetails["currency"] } }))} value={applicationDraft.offerDetails.currency}>{["ILS", "USD", "EUR", "GBP", "Other"].map((currency) => <option key={currency}>{currency}</option>)}</select></Field><Field label={language === "he" ? "שכר בסיס מוצע" : "Offered base salary"}><input className="form-control" inputMode="decimal" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, baseSalary: event.target.value } }))} value={applicationDraft.offerDetails.baseSalary} /></Field><Field label={language === "he" ? "בונוס / משתנה" : "Bonus / variable"}><input className="form-control" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, bonus: event.target.value } }))} value={applicationDraft.offerDetails.bonus} /></Field></div><div className="grid gap-4 sm:grid-cols-2"><Field label={language === "he" ? "הון / מניות" : "Equity"}><input className="form-control" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, equity: event.target.value } }))} value={applicationDraft.offerDetails.equity} /></Field><Field label={language === "he" ? "תפקיד או דרגה מוצעים" : "Offered title or level"}><input className="form-control" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, titleLevel: event.target.value } }))} value={applicationDraft.offerDetails.titleLevel} /></Field></div><Field label={language === "he" ? "הטבות" : "Benefits"}><textarea className="form-control min-h-20 resize-y" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, benefits: event.target.value } }))} value={applicationDraft.offerDetails.benefits} /></Field><div className="grid gap-4 sm:grid-cols-3"><Field label={language === "he" ? "תאריך התחלה" : "Start date"}><input className="form-control" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, startDate: event.target.value } }))} type="date" value={applicationDraft.offerDetails.startDate} /></Field><Field label={language === "he" ? "מועד החלטה רשמי" : "Official response deadline"}><input className="form-control" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, responseDeadline: event.target.value } }))} type="date" value={applicationDraft.offerDetails.responseDeadline} /></Field><Field label={language === "he" ? "מודל עבודה מאומת" : "Confirmed work model"}><input className="form-control" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, workModel: event.target.value } }))} value={applicationDraft.offerDetails.workModel} /></Field></div><Field label={language === "he" ? "תנאים מאומתים נוספים" : "Additional verified terms"}><textarea className="form-control min-h-20 resize-y" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, additionalTerms: event.target.value } }))} value={applicationDraft.offerDetails.additionalTerms} /></Field><Field label={language === "he" ? "תאריך אימות אחרון" : "Last verified date"}><input className="form-control" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, verifiedAt: event.target.value } }))} type="date" value={applicationDraft.offerDetails.verifiedAt} /></Field><div className="negotiation-editor"><div><p><MessagesSquare className="h-4 w-4" />{language === "he" ? "מעקב משא ומתן" : "Negotiation tracker"}</p><small>{language === "he" ? "תעדו רק בקשות ותוצאות אמיתיות. שדה ריק נשאר לא ידוע." : "Record only real requests and outcomes. A blank field remains unknown."}</small></div><div className="negotiation-editor-list">{applicationDraft.offerDetails.negotiationTopics.map((item) => <article key={item.id}><strong>{negotiationTopicLabel(item.topic, language)}</strong><select aria-label={`${negotiationTopicLabel(item.topic, language)} — ${language === "he" ? "סטטוס" : "status"}`} className="form-control" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, negotiationTopics: current.offerDetails.negotiationTopics.map((entry) => entry.id === item.id ? { ...entry, status: event.target.value as NegotiationStatus } : entry) } }))} value={item.status}>{(["Not discussed", "In discussion", "Agreed", "Declined"] as NegotiationStatus[]).map((status) => <option key={status} value={status}>{negotiationStatusLabel(status, language)}</option>)}</select><input aria-label={`${negotiationTopicLabel(item.topic, language)} — ${language === "he" ? "הצעה מקורית" : "original"}`} className="form-control" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, negotiationTopics: current.offerDetails.negotiationTopics.map((entry) => entry.id === item.id ? { ...entry, original: event.target.value } : entry) } }))} placeholder={language === "he" ? "הצעה מקורית" : "Original"} value={item.original} /><input aria-label={`${negotiationTopicLabel(item.topic, language)} — ${language === "he" ? "יעד" : "target"}`} className="form-control" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, negotiationTopics: current.offerDetails.negotiationTopics.map((entry) => entry.id === item.id ? { ...entry, target: event.target.value } : entry) } }))} placeholder={language === "he" ? "היעד שלי" : "My target"} value={item.target} /><input aria-label={`${negotiationTopicLabel(item.topic, language)} — ${language === "he" ? "תוצאה" : "outcome"}`} className="form-control" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, negotiationTopics: current.offerDetails.negotiationTopics.map((entry) => entry.id === item.id ? { ...entry, outcome: event.target.value } : entry) } }))} placeholder={language === "he" ? "מה סוכם" : "Agreed outcome"} value={item.outcome} /></article>)}</div></div><div className="decision-editor"><div><p><CheckCircle2 className="h-4 w-4" />{language === "he" ? "החלטה סופית" : "Final decision"}</p><small>{language === "he" ? "Carvio מתעד את ההחלטה שלכם ואינו מחליט במקומכם." : "Carvio records your decision and never makes it for you."}</small></div><div className="grid gap-4 sm:grid-cols-[1fr_180px]"><Field label={language === "he" ? "סטטוס החלטה" : "Decision status"}><select className="form-control" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, decisionOutcome: event.target.value as OfferDecisionOutcome } }))} value={applicationDraft.offerDetails.decisionOutcome}>{(["Undecided", "Accepted", "Declined", "More time requested"] as OfferDecisionOutcome[]).map((outcome) => <option key={outcome} value={outcome}>{offerDecisionLabel(outcome, language)}</option>)}</select></Field><Field label={language === "he" ? "תאריך החלטה" : "Decision date"}><input className="form-control" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, decisionDate: event.target.value } }))} type="date" value={applicationDraft.offerDetails.decisionDate} /></Field></div><Field label={language === "he" ? "נימוק ההחלטה" : "Decision rationale"}><textarea className="form-control min-h-20 resize-y" onChange={(event) => setApplicationDraft((current) => ({ ...current, offerDetails: { ...current.offerDetails, decisionReason: event.target.value } }))} placeholder={language === "he" ? "מה הכריע את ההחלטה? כתבו לעצמכם, לא לאלגוריתם." : "What shaped the decision? Write it for yourself, not an algorithm."} value={applicationDraft.offerDetails.decisionReason} /></Field></div></section>}
